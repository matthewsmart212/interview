...
export function selectAppJourneyState({
  ...
  // Prefer nearest upcoming; otherwise average across interviews with real prep data
  let readinessFocus = nearestReadiness;
  if (readinessFocus == null && interviews.length > 0) {
    const vals = interviews.map((iv) => calculateInterviewReadiness(iv));
    readinessFocus = Math.round(
