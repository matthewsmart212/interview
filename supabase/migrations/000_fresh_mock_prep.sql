-- ============================================================
-- Get the Job / Interview Coach — FULL schema (fresh install)
-- Project: qxybmhzwnykahvtuvmkq
--
-- Run this ONCE in Supabase → SQL Editor → New query → Run
-- https://supabase.com/dashboard/project/qxybmhzwnykahvtuvmkq/sql/new
--
-- Product model:
--   1 permanent CV per user
--   many interviews (each with optional JD)
--   many mock sessions (scores / history)
-- ============================================================
...
create table if not exists public.profiles (
  ...
  goal text check (goal is null or goal in ('interview','practice','both','apply')),
...
create table if not exists public.mock_sessions (
  ...
  context_mode text not null default 'generic', -- generic | interview
...
create policy "master_cvs_all_own" on public.master_cvs
...
create policy "mock_sessions_all_own" on public.mock_sessions
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
